...
class AddUpdateDishActivity : AppCompatActivity(), View.OnClickListener, CustomListItemAdapters.onItemClickListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mFavDishDetails?.let {
            if(it.id !=0) {
                mBinding.apply {
                    currentPhotoUri = it.image.toUri()
                    Glide
                    ...
                    btnAddDish.setText(R.string.lbl_update_dish)
                }
            }
        }
    ...
    private fun setUpActionBar() {
        ...
        mFavDishDetails?.let {
            if(it.id != 0) {
                supportActionBar?.title = "Edit Dish Details"

            }
        } ?: run {
            supportActionBar?.title = "Add Dish Details"
        }
    }
